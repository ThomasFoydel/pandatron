...
(function () {
  ...
  var styleBlackKey = function (key) {
    ...
    key.el.style.left =
      Math.floor(white_key_width * (key.noteNumber + 1) - black_key_width / 2) +
      'px';
  ...
  var createKeys = function () {
    ...
    for (i = 0; i < total_white_keys; i++) {
      ...
      let bizarre_note_counter = this.whiteNotes[note_counter];
  ...
  var addListeners = function (keyboard_element) {
    ...
    globalWindow.addEventListener('keydown', function (key) {
      ...
      if (keyboardDown(key, that.keyDown)) {
        // key.preventDefault();
        // disabled to allow for typing in auth components
      }
    });
